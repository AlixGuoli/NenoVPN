...
final class ReportCat {
    ...
    func reportConnect(moment: String, ip: String? = nil, sid: String? = nil) {
        ...
        switch moment {
        case Self.E_START:
            message = "\(Self.E_START),\(code),0.0.0.0"
        case Self.E_FAIL:
            message = "\(Self.E_FAIL),\(code),\(ip ?? "0.0.0.0")"
        case Self.E_SUCCESS:
            message = "\(Self.E_SUCCESS),0,\(code),\(ip ?? "0.0.0.0")"
        case Self.E_DISCONNECT:
            message = "\(Self.E_DISCONNECT),\(code),\(ip ?? "0.0.0.0")"
        default:
        ...
        // 打印关键信息
        let eventName = getEventName(moment)
    ...
    func reportAd(moment: String, key: String? = nil, adMoment: String? = nil) {
        ...
        let message: String
        switch moment {
        case Self.E_AD_START:
            message = "\(Self.E_AD_START),\(momentValue),\(ip),ad"
        case Self.E_AD_SUCCESS:
            message = "\(Self.E_AD_SUCCESS),\(momentValue),\(ip),ad,\(key ?? "")"
        case Self.E_AD_SHOW:
            message = "\(Self.E_AD_SHOW),\(momentValue),\(ip),ad,\(key ?? "empty")"
        default:
        ...
        // 打印关键信息
        let eventName = getAdEventName(moment)
    ...
    func reportStatus(success: Bool) {
        Task.detached { [statusURL = self.fetchStatusEndpoint()] in
            ...
            await Self.performRequest(urlString: url, messageId: messageId)
        }
    }
    ...
    private func sendLog(message: String) {
        // 提取消息标识（用于匹配请求）
        let messageId = extractMessageId(message)
        
        Task.detached { [logURL = self.fetchLogEndpoint(), msgId = messageId] in
            ...
            await Self.performRequest(urlString: url, messageId: msgId)
        }
    }
    
    private func fetchStatusEndpoint() -> String? {
    ...
    private func fetchLogEndpoint() -> String? {
        if let config = SourceVault.shared.load() ?? SourceVault.shared.ensure() {
    ...
    private static func buildURL(
        ...
        ? statusParams(value: message)
        : logParams(message: message)
        
        return components.url?.absoluteString
    }
    
    private static func statusParams(value: String) -> [URLQueryItem] {
    ...
    private static func performRequest(urlString: String, messageId: String) async {
    ...
    private func getEventName(_ moment: String) -> String {
        switch moment {
        case Self.E_START: return "开始连接"
        case Self.E_SUCCESS: return "连接成功"
        case Self.E_FAIL: return "连接失败"
        case Self.E_DISCONNECT: return "断开连接"
        default: return moment
        }
    }
    
    private func getAdEventName(_ moment: String) -> String {
        switch moment {
        case Self.E_AD_START: return "开始加载广告"
        case Self.E_AD_SUCCESS: return "广告加载成功"
        case Self.E_AD_SHOW: return "展示广告"
        default: return moment
        }
    }
    
    private func extractMessageId(_ message: String) -> String {
        ...
        // 根据事件类型提取不同的标识
        if eventType == Self.E_START || eventType == Self.E_SUCCESS || eventType == Self.E_FAIL || eventType == Self.E_DISCONNECT {
            // Connect 事件：提取时间戳部分
            if parts.count > 1 {
            ...
            return "Connect"
        } else if eventType == Self.E_AD_START || eventType == Self.E_AD_SUCCESS || eventType == Self.E_AD_SHOW {
